...
public class HomeAdapter extends RecyclerView.Adapter<HomeAdapter.ProductInHomeViewHolder> {
    ...
    public ProductInHomeViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.custome_card_home, parent, false);
    ...
    public void onBindViewHolder(@NonNull ProductInHomeViewHolder holder, int position) {
        ...
        if (p.getImage() != null && !p.getImage().isEmpty()) {
            int resId = holder.itemView.getContext().getResources()
                    .getIdentifier(p.getImage().trim().toLowerCase(), "drawable", holder.itemView.getContext().getPackageName());
        ...
        if (p.getDiscount() > 0) {
            holder.priceAfter.setText(p.getPrice() - (p.getPrice() * (p.getDiscount() / 100.0)) + "$");
